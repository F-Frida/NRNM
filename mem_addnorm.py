...
class MemoryCell(nn.Module):
    ...
    def __init__(self,
        ...
        # input concat norm, rebuttal
        self.concat_layernorm = nn.LayerNorm([self.mem_slots_plus_input, self.mem_size])

        # used for attend_over_memory function
        # must check, in tf version, mlp layer
        ...
        self.input_projector = nn.Linear(self.input_size * self.mem_slots, self.mem_size)    # check, add self.mem_slots 2019/1/29
    ...
    def multihead_attention(self, memory):
        ...
        output_transpose = output.permute(0, 2, 1, 3).contiguous()   # maybe there should be a linear projection, added 2019/1/22
        new_memory = output_transpose.view((output_transpose.shape[0], output_transpose.shape[1], -1))   # and add a [residual connection + layer normalization]
    ...
    def attend_over_memory(self, memory):
        ...
        for _ in range(self.num_blocks):
            ...
            attention_mlp = memory    # maybe should add dropout here!!!
    ...
    def forward_step(self, inputs, memory, hiddens, treat_input_as_matrix=False):
        """
        Forward step of the relational memory core.
        Args:
          inputs: Tensor input.
          memory: Memory output from the previous time step.
          hiddens: Hiddens output from LSTM Block.
        ...
        memory_plus_input = torch.cat([hiddens, inputs_reshape], dim=1)      #  differ from original RMC paper, must be carefull
        memory_plus_input = self.concat_layernorm(memory_plus_input) # add rebuttal
        ...
        next_memory = next_memory[:, :-n, :]  # minus input
        if self.gate_style == 'unit' or self.gate_style == 'memory':
            ...
            next_memory = next_memory + forget_gate * memory
            # next_memory += forget_gate * memory

        output = next_memory.view(next_memory.shape[0], -1)   # [batch, 512]

        return output, next_memory

    def forward(self, inputs, memory, hiddens, treat_input_as_matrix=False):

        output, memory = self.forward_step(inputs, memory, hiddens, treat_input_as_matrix)
...
if __name__ == '__main__':
    ...
    seq_length = 1
    batch_size = 32
    model = MemoryCell(mem_slots=1, head_size=128, input_size=input_size, num_heads=4, num_blocks=1, forget_bias=1., input_bias=0.)
    model_memory = model.initial_state(batch_size=batch_size)

    # random input
    random_input = torch.randn((32, 4, input_size))

    # take a one step forward
    output, next_memory = model(random_input, model_memory, treat_input_as_matrix=False)
    print(output.size(), next_memory.size())
